...
impl Jira {
  pub fn new(cfg: &Config, users: &str) -> Jira {
    ...
    let mut f = File::open(users).unwrap();

    f.read_to_string(&mut bytes);
    let doc = &check!(YamlLoader::load_from_str(&bytes))[0];
    let user_vec = doc["users"].to_owned().into_vec().unwrap();
  ...
  fn get_issues(sprint: &JsonValue) -> Result<(Duration, Duration, Duration), ReportError> {
    let completed = sprint["completedIssuesEstimateSum"]["value"]
      .as_fixed_point_i64(0)
      .map(Duration::seconds);
    let incomplete = sprint["incompletedIssuesEstimateSum"]["value"]
      .as_fixed_point_i64(0)
      .map(Duration::seconds);
    let total = sprint["allIssuesEstimateSum"]["value"]
      .as_fixed_point_i64(0)
      .map(Duration::seconds);
  ...
  fn issue_breakdown(&self, name: &str, issues: &JsonValue) -> String {
    ...
    let base = self
      .cfg
      .get_str("jira_base")
      .expect("No base URL in Config");
    let base_url = Url::parse(&base).unwrap();
    let url = base_url.host_str().unwrap();
    result += &format!("{} ({})\n", name, issues.len());
    if issues.len() >= 1 {
  ...
  fn jira_request(&self, rapid_id: &str, sprint_id: &str) -> JsonValue {
    let session_id = self.cfg.get_str("session_id").unwrap();
  ...
  pub fn build_report(&self, report: &JsonValue) -> String {
    ...
    result += &format!(
      "Sprint Ends in: {} day(s) on {}\n",
      time_to_end.num_days(),
      &pretty_date(end)
    );
    result += &format!(
      "Completed: {}h ({:.1}%), {} Issues\n",
      compl.num_hours(),
      comp_hours,
      completed_issues.len()
    );
    result += &format!(
      "Incomplete: {}h ({:.1}%), {} Issues\n\n",
      incompl.num_hours(),
      incomp_hours,
      incompleted_issues.len()
    );
    ...
    for u in &self.users {
      ...
      if num_issues > 0 {
        ...
        result += &format!(
          "{} has {} hours left on {} ({:.1}%) hours for {} issues\n",
